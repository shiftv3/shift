...
describe('PUT /api/dapps/transaction', function () {
	...
	it('using random secret should fail', function (done) {
		...
		putTransaction(validParams, function (err, res) {
			node.expect(res.body).to.have.property('success').to.not.be.ok;
			node.expect(res.body).to.have.property('error').to.match(/Account has no SHIFT: [0-9]+L balance=0/);
			done();
		});
	});
...
describe('PUT /api/dapps/withdrawal', function () {
	...
	beforeEach(function (done) {
		...
		var recipientId = node.shiftsk.crypto.getAddress(keys.publicKey);
		var transaction = node.shiftsk.transaction.createTransaction('1L', 100000000, account.password);
