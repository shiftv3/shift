...
describe('POST /peer/transactions', function () {
	...
	it('using incorrect nethash in headers should fail', function (done) {
		var transaction = node.shiftsk.transaction.createTransaction('1L', 1, node.gAccount.password);
	...
	it('using varying recipientId casing should go to same address', function (done) {
		...
		var keys = node.shiftsk.crypto.getKeys(account.password);
		var address = node.shiftsk.crypto.getAddress(keys.publicKey);

		var transaction = node.shiftsk.transaction.createTransaction(address, 100000000, node.gAccount.password);
		postTransaction(transaction, function (err, res) {
			node.expect(res.body).to.have.property('success').to.be.ok;

			node.onNewBlock(function (err) {
				var transaction2 = node.shiftsk.transaction.createTransaction(address.toLowerCase(), 100000000, node.gAccount.password);
	...
	it('using transaction with undefined recipientId should fail', function (done) {
		var transaction = node.shiftsk.transaction.createTransaction(undefined, 1, node.gAccount.password);
	...
	it('using invalid passphrase should fail', function (done) {
		var transaction = node.shiftsk.transaction.createTransaction('12L', 1, node.gAccount.password);
		transaction.recipientId = '1L';
		transaction.id = node.shiftsk.crypto.getId(transaction);
	...
	it('when sender has no funds should fail', function (done) {
		var transaction = node.shiftsk.transaction.createTransaction('1L', 1, 'randomstring');
	...
	it('using very large amount and genesis block id should fail', function (done) {
		var transaction = node.shiftsk.transaction.createTransaction('12L', 10000000000000000, node.gAccount.password);
	...
	it('using overflown amount should fail', function (done) {
		var transaction = node.shiftsk.transaction.createTransaction('12L', 184819291270000000012910218291201281920128129,
	...
	it('using float amount should fail', function (done) {
		var transaction = node.shiftsk.transaction.createTransaction('12L', 1.3, node.gAccount.password);
